  ...
  return (
    ...
    >
      <div className="group-hover:bg-[#353535] rounded-lg shadow-inner">
        <div className={cn("flex justify-between items-center ")}>
          {image_url && (
            ...
            >
              ...
              />
              <div className="img_overlay absolute inset-x-0 bottom-0 w-full h-[40px] bg-gradient-to-t from-[#f5f5f5] to-transparent z-10 group-hover:from-[#353535]" />
            </div>
          )}
          <div className="text flex-1 text-left pl-2 pr-4">
            {title && (
        ...
        <div className="hidden md:flex items-center gap-2 py-2 px-3">
          {categories?.map((category) => (
            <div
              key={category.id}
              className="bg-white group-hover:bg-[#444444] border border-[#eeeeee] group-hover:border-[#EEEEEE1A] rounded-full py-1 px-2"
            >
              <p className="text-muted-foreground text-xs group-hover:text-white">
                {category.name}
              </p>
            </div>
          ))}
